...
const createPzLib = ({ v3d=null, appInstance=null }) => {
    ...
    return {
        ...
        transformEulerV3dToBlenderShortest, RotationInterface, getObjectUUID, initObjectPicking,
    };
};
...
var VARS = Object.defineProperties({}, {
    ...
    'menu_index': { get: function() { return menu_index; }, set: function(val) { menu_index = val; } },
    ...
    'menu_list': { get: function() { return menu_list; }, set: function(val) { menu_list = val; } },
    ...
    'jj': { get: function() { return jj; }, set: function(val) { jj = val; } },
});

var st, x, controls, VR_mode, menu_index, walls, menu_list, state, i, j, prevStickMoved, visibility, k, move_forward, move_side, dir_vector_forward, dir_vector_side, dir_vector, move_speed, ray_cast_position, YY, raycast_result, XX, jj;
...
st = [[['GROUP', 'logo']], [['GROUP', 'tutorial_1'], ['GROUP', 'base']], [['GROUP', 'tutorial_2'], ['GROUP', 'base'], ['GROUP', 'zone error']], [['GROUP', 'tutorial_3'], ['GROUP', 'base'], ['GROUP', 'switcher'], ['GROUP', 'zone error']], [['GROUP', 'tutorial_4'], ['GROUP', 'base'], ['GROUP', 'switcher']], [['GROUP', 'tutorial_5'], ['GROUP', 'base'], ['GROUP', 'switcher']], [['GROUP', 'menu'], ['GROUP', 'zone error'], ['GROUP', 'base'], ['GROUP', 'switcher']], [['GROUP', 'menu'], ['GROUP', 'train'], ['GROUP', 'base'], ['GROUP', 'switcher']], [['GROUP', 'menu'], ['GROUP', 'train'], ['GROUP', 'base'], ['GROUP', 'switcher']], [['GROUP', 'menu'], ['GROUP', 'train'], ['GROUP', 'base'], ['GROUP', 'switcher']], [['GROUP', 'menu'], ['GROUP', 'train'], ['GROUP', 'base'], ['GROUP', 'switcher']], [['GROUP', 'menu'], ['GROUP', 'base'], ['GROUP', 'switcher'], ['GROUP', 'only_gizmo']]];
menu_index = 0;
menu_list = ['menu_0', 'menu_1', 'menu_2', 'menu_3', 'menu_4'];
state = 0;
    ...
    traverseControllers(function() {
        if ((_pGlob.gamepadIndex || 0) == 0) {
            ...
            if ((Math.abs(YY) > 0.5 || Math.abs(XX) > 0.5) && !prevStickMoved) {
                ...
                if (isObjectVisible(['GROUP', 'menu'])) {
                    for (var jj_index in menu_list) {
                        let jj = menu_list[jj_index];
                        changeVis(jj, false);
                    }
                    changeVis(menu_list[menu_index], true);
                }
                var i_list = st[state];
...
for (var k_index in menu_list) {
    let k = menu_list[k_index];
    registerOnClick(k, false, false, [0,1,2], false, function(event) {
        menu_index = (menu_index + 1) % menu_list.length;
        for (var j_index2 in menu_list) {
            let j = menu_list[j_index2];
            changeVis(j, false);
        }
        changeVis(menu_list[menu_index], true);
    }, function(event) {});
