...
const createPzLib = ({ v3d=null, appInstance=null }) => {
    ...
    return {
        ...
        transformEulerV3dToBlenderShortest, RotationInterface, getMaterialEditableColors, initObjectPicking,
        getObjectUUID,
    };
};
...
var st, controls, VR_mode, state, walls, prevStickMoved, j, visibility, blue, move_forward, YY, move_side, XX, dir_vector_forward, dir_vector_side, dir_vector, move_speed, ray_cast_position, raycast_result;
...
st = [['GROUP', 'logo'], ['GROUP', 'base'], ['GROUP', 'train'], ['GROUP', 'red_line']];
state = 0;
    ...
    traverseControllers(function() {
        ...
        if ((_pGlob.gamepadIndex || 0) == 1) {
            ...
            if ((Math.abs(YY) > 0.5 || Math.abs(XX) > 0.5) && !prevStickMoved) {
                ...
                changeVis(getObjectsFrom(getObjectsFrom(st, 'ALL', 'null', undefined, false), 'ALL', 'null', undefined, false), false);
                state = (state + 1) % st.length;
                changeVis(getObjectsFrom(st[state], 'ALL', 'null', undefined, false), true);
                prevStickMoved = true;
