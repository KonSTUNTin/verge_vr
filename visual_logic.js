...
const createPzLib = ({ v3d=null, appInstance=null }) => {
    ...
    return {
        ...
        unbindListener, getObjectByName, isMeshObject, isObjectWorthProcessing,
        xrGetInputSource, LIST_NONE, transformCoordsSpace, getSceneCoordSystem,
        transformEulerV3dToBlenderShortest, RotationInterface, initObjectPicking, getObjectUUID,
    };
};
...
this.procedures["change_state"] = change_state;
this.procedures["blue_hover"] = blue_hover;
this.procedures["red_hover"] = red_hover;
...
var VARS = Object.defineProperties({}, {
    'st': { get: function() { return st; }, set: function(val) { st = val; } },
    'x': { get: function() { return x; }, set: function(val) { x = val; } },
    'y': { get: function() { return y; }, set: function(val) { y = val; } },
    'VR_mode': { get: function() { return VR_mode; }, set: function(val) { VR_mode = val; } },
    'state': { get: function() { return state; }, set: function(val) { state = val; } },
    'color_operative': { get: function() { return color_operative; }, set: function(val) { color_operative = val; } },
    'color_operative_2': { get: function() { return color_operative_2; }, set: function(val) { color_operative_2 = val; } },
    'prevStickMoved': { get: function() { return prevStickMoved; }, set: function(val) { prevStickMoved = val; } },
    'i': { get: function() { return i; }, set: function(val) { i = val; } },
    'visibility': { get: function() { return visibility; }, set: function(val) { visibility = val; } },
    'j': { get: function() { return j; }, set: function(val) { j = val; } },
    'walls': { get: function() { return walls; }, set: function(val) { walls = val; } },
    'move forward': { get: function() { return move_forward; }, set: function(val) { move_forward = val; } },
    'move side': { get: function() { return move_side; }, set: function(val) { move_side = val; } },
    'dir vector forward': { get: function() { return dir_vector_forward; }, set: function(val) { dir_vector_forward = val; } },
    'dir vector side': { get: function() { return dir_vector_side; }, set: function(val) { dir_vector_side = val; } },
    'dir vector': { get: function() { return dir_vector; }, set: function(val) { dir_vector = val; } },
    'move speed': { get: function() { return move_speed; }, set: function(val) { move_speed = val; } },
    'ray cast position': { get: function() { return ray_cast_position; }, set: function(val) { ray_cast_position = val; } },
    'raycast result': { get: function() { return raycast_result; }, set: function(val) { raycast_result = val; } },
    'YY': { get: function() { return YY; }, set: function(val) { YY = val; } },
    'XX': { get: function() { return XX; }, set: function(val) { XX = val; } },
    'controls': { get: function() { return controls; }, set: function(val) { controls = val; } },
});

var st, x, y, controls, VR_mode, state, color_operative, color_operative_2, walls, prevStickMoved, i, j, visibility, move_forward, move_side, dir_vector_forward, dir_vector_side, dir_vector, move_speed, ray_cast_position, raycast_result, YY, XX;
...
function change_state(x) {
    if (state + x >= 0) {
        state = (state + x) % st.length;
    }
    for (var i_index3 in st) {
        let i = st[i_index3];
        for (var j_index2 in i) {
            let j = i[j_index2];
            changeVis(getObjectsFrom(j, 'ALL', 'null', undefined, false), false);
        }
    }
    var i_list2 = st[state];
    for (var i_index4 in i_list2) {
        let i = i_list2[i_index4];
        changeVis(getObjectsFrom(i, 'ALL', 'null', undefined, false), true);
    }
    var j_end2 = walls.length - 1;
    var j_inc2 = 1;
    if (0 > j_end2) {
        j_inc2 = -j_inc2;
    }
    for (let j = 0; j_inc2 >= 0 ? j <= j_end2 : j >= j_end2; j += j_inc2) {
        changeVis(walls[j], false);
    }
}
...
// Describe this function...
function blue_hover(x) {
    color_operative = getObjectMaterial(x, undefined, false);
    registerOnHover(x, false, function(event) {
        assignMat(x, 'Blue_hov', undefined);
    }, function(event) {
        assignMat(x, color_operative, undefined);
    });
}

// Describe this function...
function red_hover(y) {
    color_operative_2 = getObjectMaterial(y, undefined, false);
    registerOnHover(y, false, function(event) {
        assignMat(y, 'cold red hover', undefined);
    }, function(event) {
        assignMat(y, color_operative_2, undefined);
    });
}
...
st = [[['GROUP', 'logo']], [['GROUP', 'zone error'], ['GROUP', 'base']], [['GROUP', 'zone error 2'], ['GROUP', 'base']], [['GROUP', 'menu_5'], ['GROUP', 'base'], ['GROUP', 'switcher'], ['GROUP', 'only_gizmo'], ['GROUP', 'hover']], [['GROUP', 'tutorial_1'], ['GROUP', 'base']], [['GROUP', 'tutorial_2'], ['GROUP', 'base'], ['GROUP', 'zone error']], [['GROUP', 'tutorial_3'], ['GROUP', 'base'], ['GROUP', 'zone error']], [['GROUP', 'tutorial_4'], ['GROUP', 'base'], ['GROUP', 'switcher']], [['GROUP', 'tutorial_5'], ['GROUP', 'base'], ['GROUP', 'switcher']], [['GROUP', 'tutorial_6'], ['GROUP', 'base'], ['GROUP', 'switcher']], [['GROUP', 'tutorial_7'], ['GROUP', 'base'], ['GROUP', 'switcher']], [['GROUP', 'menu_0'], ['GROUP', 'zone error'], ['GROUP', 'base'], ['GROUP', 'switcher']], [['GROUP', 'menu_1'], ['GROUP', 'zone error 2'], ['GROUP', 'base'], ['GROUP', 'switcher']], [['GROUP', 'menu_2'], ['GROUP', 'train'], ['GROUP', 'base'], ['GROUP', 'switcher']], [['GROUP', 'menu_3'], ['GROUP', 'train'], ['GROUP', 'base'], ['GROUP', 'switcher']], [['GROUP', 'menu_4'], ['GROUP', 'train'], ['GROUP', 'base'], ['GROUP', 'switcher']], [['GROUP', 'menu_4'], ['GROUP', 'base'], ['GROUP', 'switcher'], ['GROUP', 'only_gizmo'], ['GROUP', 'hover']]];
state = 0;
prevStickMoved = false;
visibility = true;
changeVis(getObjectsFrom(st, 'ALL', 'null', undefined, false), false);
changeVis(st[0], true);
registerEveryFrame(function() {
    traverseControllers(function() {
        if ((_pGlob.gamepadIndex || 0) == 0) {
            makeParent('menu_0', xrControllerProp('RAY_RETICLE'));
            makeParent('menu_1', xrControllerProp('RAY_RETICLE'));
            makeParent('menu_2', xrControllerProp('RAY_RETICLE'));
            makeParent('menu_3', xrControllerProp('RAY_RETICLE'));
            makeParent('menu_4', xrControllerProp('RAY_RETICLE'));
            makeParent('menu_5', xrControllerProp('RAY_RETICLE'));
            makeParent('tutorial_1', xrControllerProp('RAY_RETICLE'));
            makeParent('tutorial_2', xrControllerProp('RAY_RETICLE'));
            makeParent('tutorial_3', xrControllerProp('RAY_RETICLE'));
            makeParent('tutorial_4', xrControllerProp('RAY_RETICLE'));
            makeParent('tutorial_5', xrControllerProp('RAY_RETICLE'));
            makeParent('tutorial_6', xrControllerProp('RAY_RETICLE'));
            makeParent('tutorial_7', xrControllerProp('RAY_RETICLE'));
            YY = getGamepadProp(xrControllerProp('GAMEPAD_INDEX'), 'AXIS', '0');
            XX = getGamepadProp(xrControllerProp('GAMEPAD_INDEX'), 'AXIS', '2');
            if ((Math.abs(YY) > 0.5 || Math.abs(XX) > 0.5) && !prevStickMoved) {
                prevStickMoved = !prevStickMoved;
                state = (state + 1) % st.length;
                for (var i_index in st) {
                    let i = st[i_index];
                    for (var j_index in i) {
                        let j = i[j_index];
                        changeVis(getObjectsFrom(j, 'ALL', 'null', undefined, false), false);
                    }
                }
                ...
                for (var i_index2 in i_list) {
                    let i = i_list[i_index2];
                    changeVis(getObjectsFrom(i, 'ALL', 'null', undefined, false), true);
                }
                var j_end = walls.length - 1;
                var j_inc = 1;
                if (0 > j_end) {
                    j_inc = -j_inc;
                }
                for (let j = 0; j_inc >= 0 ? j <= j_end : j >= j_end; j += j_inc) {
                    changeVis(walls[j], false);
                }
                prevStickMoved = true;
...
'        puzzles.procedures[\'change_state\'](-1);' + '\n' +
...
controls = ['vr wall_1', 'vr wall_2', 'vr wall_3', 'vr wall_4'];
walls = ['wall_1', 'wall_2', 'wall_3', 'wall_4'];
var j_end3 = walls.length - 1;
var j_inc3 = 1;
if (0 > j_end3) {
    j_inc3 = -j_inc3;
}
for (let j = 0; j_inc3 >= 0 ? j <= j_end3 : j >= j_end3; j += j_inc3) {
    registerOnClick(controls[j], false, false, [0,1,2], false, function(event) {
        if (isObjectVisible(walls[j])) {
            changeVis(walls[j], false);
        } else {
            changeVis(walls[j], true);
        }
    }, function(event) {});
}
